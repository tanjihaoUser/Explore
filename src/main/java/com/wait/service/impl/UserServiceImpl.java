        ...
        try {
            ...
            for (UserBase user : allUsers) {
                ...
                Long postCount = postStats != null && postStats.get("postCount") != null 
                    ? ((Number) postStats.get("postCount")).longValue() : 0L;
                Long totalLikeCount = postStats != null && postStats.get("totalLikeCount") != null 
                    ? ((Number) postStats.get("totalLikeCount")).longValue() : 0L;
                Long totalCommentCount = postStats != null && postStats.get("totalCommentCount") != null 
                    ? ((Number) postStats.get("totalCommentCount")).longValue() : 0L;

                // 从Redis获取收藏总数（需要获取用户的所有帖子ID，然后统计收藏数）
                Long totalFavoriteCount = 0L;
                try {
                    List<com.wait.entity.domain.Post> userPosts = postMapper.selectByUserId(userId);
                    if (userPosts != null && !userPosts.isEmpty()) {
                        for (com.wait.entity.domain.Post post : userPosts) {
                            if (post.getId() != null) {
                ...
                double score = postCount * 1.0 + totalLikeCount * 2.0 + totalFavoriteCount * 3.0 + totalCommentCount * 1.0;
